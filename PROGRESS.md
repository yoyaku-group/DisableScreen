...
- [DEFECT:contradiction] — **RÉSOLU (2026-09-20, ADR 020)** : `runclosed doctor` rapportait `lid.backend="unqualified"` (hardcodé) alors que `docs/compatibility/README.md` enregistre le chemin capot **PASS (2026-09-20)**. Le doctor rapporte maintenant deux faits distincts (`lid.backend` = chemin réel `sudo -n /usr/bin/pmset` ; `lid.qualification` = qualifié live + prérequis sudoers ; `lid.runLid` = stub ADR 007) et le refus `run --lid` est reformulé (« CLI lid wrapper not implemented », plus « backend unqualified »).
