...
- DONE — CLI `runclosed disable <id>` / `runclosed enable <id>` / `runclosed restore --owned`. Exit codes : 0 succès / 3 refusé (stale / last-active / unknown) / 5 backend failure / 64 usage.
...
- DONE — **défaut #3 résolu** : `PROGRESS.md §G3` annonçait toujours `restore --owned = stub` alors que G2b l'a implémenté en Phase 1. Mis à jour : `restore --owned` = **G2b live** (référence au §G2b-corrections), `restore --lid` reste **G3 stub**.
